...
control_plot_fig3 <- function(data, lt_var_name, lt_var_levels, lt_labeller,
                              ...
                              coord_ratio = 2.5*0.8, legend_pos = c(1,1),
...
reff_plot_fig3 <- function(data, lt_var_name, lt_var_levels, lt_labeller,
                           ...
                           coord_ratio = 1*0.8, legend_pos = c(0.01, 1), 
                           legend_vspace = -1, ttype_exclude = NULL,
...
prepare_fig3_baseline <- function(median_data,
  ...
  data_filtered <- lapply(data, function(d) 
    filter(d, p_traced_manual %in% c(0, p_traced),
...
prepare_fig3_all <- function(median_data, optim_data, pessim_data,
                            ...
                            ttype_exclude = NULL, pt_size = 2,
                            err_width_ctrl = 0.05,
                            coord_ratio_ctrl = 2.5*0.8, legend_pos_ctrl = c(1,1),
                            coord_ratio_reff = 0.625*0.8, legend_pos_reff = c(0.01, 1),
                            y_limits_reff = c(0, 4), y_title_short_reff = TRUE,
                            legend_vspace = -1, col_palette = "Set1"
){
  if (!xor(is.null(contact_limit),is.null(bt_distance))){
...
fig3_all <- prepare_fig3_all(median_data, optim_data, pessim_data,
                             0.9, 7, NULL, ttype_exclude = ttype_exclude,
                             y_title_short_reff = FALSE)
...
path_prefix <- "figures/img/fig3_"
row_height <- 16

save_fig(path_prefix, "all_7d_090.svg", fig3_all, row_height*3*0.85, 2/3 * 0.9)
