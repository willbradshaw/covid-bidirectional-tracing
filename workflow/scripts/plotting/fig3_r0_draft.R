...
r0_path <- "figure_data/fig3/r0_200_sc.tsv.gz"
r0_data <- suppressMessages(read_tsv(r0_path))

#==============================================================================
# Process data
#==============================================================================

r0_data_processed <- r0_data %>%
...
control_plot_90 <- r0_data_processed %>% filter(p_traced_auto != 0.8,
                                           p_traced_manual != 0.8) %>%
  ...
  facet_grid(.~contact_limit_manual, labeller=labeller(backtrace_distance=label_backtrace,
  ...
  theme_base +
  ggtitle("Control rates with varying R0 (90% contacts traced, 200 runs)")
  
control_plot_80 <- r0_data_processed %>% filter(p_traced_auto != 0.9,
                                                p_traced_manual != 0.9) %>%
  ggplot(aes(x=r0_base, y=p_controlled, 
             ymin=p_controlled_lower, ymax=p_controlled_upper,
             colour = trace_type,
  ...
  coord_fixed(ratio=2.5) +
  theme_base +
  ggtitle("Control rates with varying R0 (80% contacts traced, 200 runs)")

# R_eff plots

r_eff_plot_90 <- r0_data_processed %>% filter(p_traced_auto != 0.8,
                                                p_traced_manual != 0.8) %>%
  ...
  scale_linetype_discrete(name=NULL, labels=label_backtrace) +
  scale_shape_discrete(name=NULL, labels=label_backtrace) +
  guides(colour=guide_legend(nrow=4, order=1),
         linetype=guide_legend(nrow=2, order=2),
         shape=guide_legend(nrow=2, order=2)
  ) +
  coord_fixed(ratio=0.8) +
  theme_base + theme(legend.position = "right") +
  ggtitle("Effective R with varying R0 (90% contacts traced, 200 runs)")

r_eff_plot_80 <- r0_data_processed %>% filter(p_traced_auto != 0.9,
                                              p_traced_manual != 0.9) %>%
  ggplot(aes(x=r0_base, y=effective_r0_mean, 
             colour = trace_type,
             linetype = factor(backtrace_distance),
  ...
  coord_fixed(0.8) +
  theme_base + theme(legend.position = "right") +
  ggtitle("Effective R with varying R0 (80% contacts traced, 200 runs)")
