...
envir_path <- "figure_data/fig3/envir_200_sc.tsv.gz"
envir_data <- suppressMessages(read_tsv(envir_path))

#==============================================================================
# Process data
#==============================================================================

envir_data_processed <- envir_data %>%
...
control_plot_90 <- envir_data_processed %>% filter(p_traced_auto != 0.8,
                                           p_traced_manual != 0.8) %>%
  ggplot(aes(x=p_environmental, y=p_controlled, 
  ...
  scale_x_continuous(name = "% environmental transmission", limits = c(0,0.5),
  ...
  theme_base +
  ggtitle("Control rates with varying environmental\ntransmission levels (90% contacts traced, 200 runs)")
  
control_plot_80 <- envir_data_processed %>% filter(p_traced_auto != 0.9,
                                                   p_traced_manual != 0.9) %>%
  ggplot(aes(x=p_environmental, y=p_controlled, 
             ymin=p_controlled_lower, ymax=p_controlled_upper,
             colour = trace_type,
  ...
  coord_fixed(ratio=0.3) +
  theme_base +
  ggtitle("Control rates with varying environmental\ntransmission levels (80% contacts traced, 200 runs)")

# R_eff plots

r_eff_plot_90 <- envir_data_processed %>% filter(p_traced_auto != 0.8,
                                                p_traced_manual != 0.8) %>%
  ggplot(aes(x=p_environmental, y=effective_r0_mean, 
             colour = trace_type,
             linetype = factor(backtrace_distance),
             shape = factor(backtrace_distance),
  ...
  scale_x_continuous(name = "% environmental transmission", limits = c(0,0.52),
                     breaks = seq(0,1,0.1), labels = function(x) round(x*100)) +
  scale_colour_brewer(type = "div", palette = "Set1", name="Trace type") +
  scale_linetype_discrete(name=NULL, labels=label_backtrace) +
  scale_shape_discrete(name=NULL, labels=label_backtrace) +
  ...
  theme_base +
  ggtitle("Effective R with varying environmental\ntransmission levels (90% contacts traced, 200 runs)")


r_eff_plot_80 <- envir_data_processed %>% filter(p_traced_auto != 0.9,
                                                 p_traced_manual != 0.9) %>%
  ggplot(aes(x=p_environmental, y=effective_r0_mean, 
             colour = trace_type,
             linetype = factor(backtrace_distance),
...
plot_scale <- 10
plot_prefix <- "figures/fig3/p_envir_"
