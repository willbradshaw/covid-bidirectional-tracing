...
presym_path <- "figure_data/fig3/presym_200_sc.tsv.gz"
presym_data <- suppressMessages(read_tsv(presym_path))
...
alpha_to_presym <- function(alpha){
  ...
  presym[alpha == 1.4]  <- 0.2
  presym[alpha == 0.7]  <- 0.3
  presym[alpha == 0.3]  <- 0.4
  presym[alpha == 0]    <- 0.5
  presym[alpha == -0.3] <- 0.6
  presym[alpha == -0.7] <- 0.7
  presym[alpha == -1.4] <- 0.8
  return(presym)
}

presym_data_processed <- presym_data %>%
...
control_plot_90 <- presym_data_processed %>% filter(p_traced_auto != 0.8,
                                           p_traced_manual != 0.8) %>%
  ...
  scale_x_continuous(name = "% pre-symptomatic transmission", limits = c(0.2,0.8),
                     breaks = seq(0,1,0.2), labels = function(x) round(x*100)) +
  ...
  theme_base +
  ggtitle("Control rates with varying pre-symptomatic\ntransmission levels (90% contacts traced, 200 runs)")
  
control_plot_80 <- presym_data_processed %>% filter(p_traced_auto != 0.9,
                                                    p_traced_manual != 0.9) %>%
  ggplot(aes(x=p_presymptomatic, y=p_controlled, 
             ymin=p_controlled_lower, ymax=p_controlled_upper,
             colour = trace_type,
...
r_eff_plot_90 <- presym_data_processed %>% filter(p_traced_auto != 0.8,
                                                p_traced_manual != 0.8) %>%
  ...
  theme_base +
  ggtitle("Effective R with varying pre-symptomatic\ntransmission levels (90% contacts traced, 200 runs)")


r_eff_plot_80 <- presym_data_processed %>% filter(p_traced_auto != 0.9,
                                                  p_traced_manual != 0.9) %>%
  ggplot(aes(x=p_presymptomatic, y=effective_r0_mean, 
             colour = trace_type,
             linetype = factor(backtrace_distance),
...
plot_scale <- 10
plot_prefix <- "figures/fig3/p_presym_"
