...
# Read in data
scenario_path <- "saved_data/day_delay_scenario.tsv.gz"
...
label_delay <- function(y){
  paste0(round(as.numeric(y)/7), " week", ifelse(as.numeric(y)==7, "", "s"))
}
g <- ggplot(scenario_data, aes(x=p_traced_auto, y=p_controlled,
  ...
  facet_grid(p_asymptomatic~rollout_delay_days,
             labeller = labeller(
               p_asymptomatic = label_asym,
               rollout_delay_days = label_delay
               )) +
...
ggsave(filename="saved_data/test_delay_days.png",
        plot = g, device="png",
