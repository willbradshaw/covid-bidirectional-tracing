...
asym_path <- "figure_data/fig3/asym_200_sc.tsv.gz"
...
control_plot_90 <- asym_data_processed %>% filter(p_traced_auto != 0.8,
                                           p_traced_manual != 0.8) %>%
  ...
  theme_base +
  ggtitle("Control rates with varying asymptomatic carrier rates\n(90% contacts traced, 200 runs)")
  
control_plot_80 <- asym_data_processed %>% filter(p_traced_auto != 0.9,
                                                  p_traced_manual != 0.9) %>%
  ggplot(aes(x=p_asymptomatic, y=p_controlled, 
             ymin=p_controlled_lower, ymax=p_controlled_upper,
             colour = trace_type,
...
r_eff_plot_90 <- asym_data_processed %>% filter(p_traced_auto != 0.8,
                                                p_traced_manual != 0.8) %>%
  ...
  theme_base +
  ggtitle("Effective R with varying asymptomatic carrier rates\n(90% contacts traced, 200 runs)")


r_eff_plot_80 <- asym_data_processed %>% filter(p_traced_auto != 0.9,
                                                p_traced_manual != 0.9) %>%
  ggplot(aes(x=p_asymptomatic, y=effective_r0_mean, 
             colour = trace_type,
             linetype = factor(backtrace_distance),
...
base_height <- 5
ggsave(filename="figures/fig3/p_asym_control_90.png", plot = control_plot_90,
       device = "png", width = base_height*4,
       height = base_height*7, units = "cm",
       dpi = 320, limitsize = FALSE)
ggsave(filename="figures/fig3/p_asym_control_80.png", plot = control_plot_80,
       device = "png", width = base_height*4,
       height = base_height*7, units = "cm",
       dpi = 320, limitsize = FALSE)
ggsave(filename="figures/fig3/p_asym_r_eff_90.png", plot = r_eff_plot_90,
       device = "png", width = base_height*4,
       height = base_height*7, units = "cm",
       dpi = 320, limitsize = FALSE)
ggsave(filename="figures/fig3/p_asym_r_eff_80.png", plot = r_eff_plot_80,
       device = "png", width = base_height*4,
       height = base_height*7, units = "cm",
       dpi = 320, limitsize = FALSE)
...
control_plot_90 <- asym_data_processed %>% filter(p_traced_auto != 0.8,
                                                  p_traced_manual != 0.8) %>%
  ggplot(aes(x=rel_r0_asymptomatic, y=p_controlled, 
             ymin=p_controlled_lower, ymax=p_controlled_upper,
             colour = trace_type,
             linetype = factor(backtrace_distance),
             shape = factor(backtrace_distance))) +
  geom_errorbar(width=0.05) +
  geom_line() + geom_point(size=2) +
  facet_grid(p_asymptomatic~contact_limit_manual,
             labeller=labeller(backtrace_distance=label_backtrace,
                               contact_limit_manual=label_limits_manual,
                               p_asymptomatic = label_p_asym)) +
  scale_y_continuous(name = "% of outbreaks controlled", limits = c(0,1),
                     breaks = seq(0,1,0.2), labels = function(x) round(x*100)) +
  scale_x_continuous(name = "Relative R0 of asymptomatic\ncarriers", limits = c(0,1),
                     breaks = seq(0,1,0.2), labels = function(x) round(x*100)) +
  scale_colour_brewer(type = "div", palette = "Set1", name="Trace type") +
  scale_linetype_discrete(name=NULL, labels=label_backtrace) +
  scale_shape_discrete(name=NULL, labels=label_backtrace) +
  ...
  coord_fixed(ratio=0.5) +
  theme_base +
  ggtitle("Control rates with varying asymptomatic infectiousness\n(90% contacts traced, 200 runs)")
...
r_eff_plot_90 <- asym_data_processed %>% filter(p_traced_auto != 0.8,
                                                p_traced_manual != 0.8) %>%
  ggplot(aes(x=rel_r0_asymptomatic, y=effective_r0_mean, 
             colour = trace_type,
             linetype = factor(backtrace_distance),
             shape = factor(backtrace_distance),
             ymin = effective_r0_mean-effective_r0_sd,
  ...
  coord_fixed(ratio=0.3) +
  theme_base +
  ggtitle("Effective R with varying asymptomatic infectiousness\n(90% contacts traced, 200 runs)")
...
base_height <- 5
ggsave(filename="figures/fig3/rel_asym_control_90.png", plot = control_plot_90,
       device = "png", width = base_height*4,
       height = base_height*7, units = "cm",
       dpi = 320, limitsize = FALSE)
ggsave(filename="figures/fig3/rel_asym_control_80.png", plot = control_plot_80,
       device = "png", width = base_height*4,
       height = base_height*7, units = "cm",
       dpi = 320, limitsize = FALSE)
ggsave(filename="figures/fig3/rel_asym_r_eff_90.png", plot = r_eff_plot_90,
       device = "png", width = base_height*4,
       height = base_height*7, units = "cm",
       dpi = 320, limitsize = FALSE)
ggsave(filename="figures/fig3/rel_asym_r_eff_80.png", plot = r_eff_plot_80,
       device = "png", width = base_height*4,
       height = base_height*7, units = "cm",
       dpi = 320, limitsize = FALSE)