...
sensitivity_path <- "figure_data/fig3/sensitivity_200_sc.tsv.gz"
sensitivity_data <- suppressMessages(read_tsv(sensitivity_path))

#==============================================================================
# Process data
#==============================================================================

sensitivity_data_processed <- sensitivity_data %>%
  mutate(trace_type = ifelse(p_traced_auto == 0,
                             ...
                             ifelse(p_traced_manual == 0, "Automated only", 
                                    "Manual + automated")),
         test_type = ifelse(test_serological,
                            ifelse(trace_neg_symptomatic, "Active and past cases\nTest for release",
                                   "Active and past cases\nTest for tracing"),
         ifelse(trace_neg_symptomatic, "Active cases only\nTest for release",
                "Active cases only\nTest for tracing"))
         )
...
label_test <- function(x){
  ifelse(x, "Active and past cases", "Active cases only")
}
...
control_plot_90 <- sensitivity_data_processed %>% filter(p_traced_auto != 0.8,
                                           p_traced_manual != 0.8) %>%
  ...
  theme_base +
  ggtitle("Control rates with varying test sensitivity\nand test type (90% contacts traced, 200 runs)")
  
control_plot_80 <- sensitivity_data_processed %>% filter(p_traced_auto != 0.9,
                                                         p_traced_manual != 0.9) %>%
  ggplot(aes(x=test_sensitivity, y=p_controlled, 
             ymin=p_controlled_lower, ymax=p_controlled_upper,
             colour = trace_type,
  ...
  coord_fixed(ratio=0.4) +
  theme_base +
  ggtitle("Control rates with varying test sensitivity\nand test type (80% contacts traced, 200 runs)")

# R_eff plots

r_eff_plot_90 <- sensitivity_data_processed %>% filter(p_traced_auto != 0.8,
                                                p_traced_manual != 0.8) %>%
  ...
  theme_base +
  ggtitle("Effective R with varying test sensitivity\nand test type (90% contacts traced, 200 runs)")


r_eff_plot_80 <- sensitivity_data_processed %>% filter(p_traced_auto != 0.9,
                                                       p_traced_manual != 0.9) %>%
  ggplot(aes(x=test_sensitivity, y=effective_r0_mean, 
             colour = trace_type,
             linetype = factor(backtrace_distance),
