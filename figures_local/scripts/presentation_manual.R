...
ctrl_plot <- function(data, err_width = 0.05, pt_size = 2,
                         coord_ratio = 1){
  ggplot(data, aes(x=p_traced_manual, y=p_controlled,
                   ymin=p_controlled_lower, ymax=p_controlled_upper,
                   colour=factor(backtrace_distance),
                   linetype = factor(contact_limit_manual, levels=c(2,7)),
                   shape = factor(contact_limit_manual, levels=c(2,7)))) +
...
reff_plot <- function(data, pt_size = 2, coord_ratio = 1){
  ggplot(data, aes(x=p_traced_manual, y=effective_r0_mean,
                   colour=factor(backtrace_distance),
                   linetype = factor(contact_limit_manual, levels=c(2,7)),
                   shape = factor(contact_limit_manual, levels=c(2,7)))) +
...
# 2-day forward-only
manual_2day_fwd <- manual_data %>% 
  filter(p_traced_auto == 0, backtrace_distance == 0, contact_limit_manual == 2) %>%
  combo_plot

# 2-day bidirectional
manual_2day_bidir <- manual_data %>% 
  filter(p_traced_auto == 0, contact_limit_manual == 2) %>%
  combo_plot

# 7-day bidirectional
manual_7day_bidir <- manual_data %>% filter(p_traced_auto == 0) %>%
  combo_plot
...
path_prefix <- "figures_local/img/presentation_manual"

# 48% pre-symptomatic
row_height <- 14
save_fig(path_prefix, "_blank.svg", manual_2day_fwd,
         row_height, 1.5, device="svg")
save_fig(path_prefix, "_2day_fwd.png", manual_2day_fwd,
         row_height, 1.5, device="png")
save_fig(path_prefix, "_2day_bidir.png", manual_2day_bidir,
         row_height, 1.5, device="png")
save_fig(path_prefix, "_7day_bidir.png", manual_7day_bidir,
         row_height, 1.5, device="png")