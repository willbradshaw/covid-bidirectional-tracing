...
data_path <- "figures_local/data/si_sensitivity_1000_scenario.tsv.gz"
...
ctrl_plot <- function(data, err_width = 0.05, pt_size = 2,
                      coord_ratio = 1){
  ggplot(data, aes(x=test_sensitivity, y=p_controlled,
...
reff_plot <- function(data, pt_size = 2, coord_ratio = 1){
  ggplot(data, aes(x=test_sensitivity, y=effective_r0_mean,
                   colour=trace_type,
                   linetype = backtrace_distance,
                   shape = backtrace_distance)) +
...
# High-uptake
plot_high_pre <- data_processed %>% 
  filter(trace_neg_symptomatic == TRUE, p_smartphone_overall == 0.8) %>%
  combo_plot()
plot_high_post <- data_processed %>% 
  filter(trace_neg_symptomatic == FALSE, p_smartphone_overall == 0.8) %>%
  combo_plot()

# Low-uptake
plot_low_pre <- data_processed %>% 
  filter(trace_neg_symptomatic == TRUE, p_smartphone_overall == 0.53) %>%
  combo_plot()
plot_low_post <- data_processed %>% 
  filter(trace_neg_symptomatic == FALSE, p_smartphone_overall == 0.53) %>%
  combo_plot()
...
path_prefix <- "figures_local/img/presentation_sensitivity"

plot_scale <- 13
plot_ratio <- 21/plot_scale 

save_fig(path_prefix, "_high_pre.png", plot_high_pre,
         plot_scale, plot_ratio, device="png")
save_fig(path_prefix, "_high_post.png", plot_high_post,
         plot_scale, plot_ratio, device="png")
save_fig(path_prefix, "_low_pre.png", plot_low_pre,
         plot_scale, plot_ratio, device="png")
save_fig(path_prefix, "_low_post.png", plot_low_post,
         plot_scale, plot_ratio, device="png")
