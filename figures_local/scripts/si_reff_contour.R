...
make_palette <- function(data, bt_dist, col_high, step_size){
  # Calibrate range of palette
  data_range <- data %>% filter(backtrace_distance == bt_dist) %>%
    pull(effective_r0_mean)
  palette_max <- data_range %>% max %>% (function(x) ceiling(x*10)/10)
  palette_min <- data_range %>% min %>% (function(x) floor(x*10)/10)
...
bidir_palette_dense <- make_palette(contour_data, Inf, col_bidir, step_dense)
bidir_palette_sparse <- make_palette(contour_data, Inf, col_bidir, step_sparse)
fwd_palette_dense <- make_palette(contour_data, Inf, col_fwd, step_dense)
fwd_palette_sparse <- make_palette(contour_data, Inf, col_fwd, step_sparse)
...
# Digital-only
contour_bidir_digital <- contour_data %>% filter(contact_limit_manual == 7) %>% 
  contour_plot_reff(FALSE, Inf, bidir_palette_sparse, 0, 0.1) +
  ggtitle("Bidirectional digital\ntracing")
contour_fwd_digital  <- contour_data %>% filter(contact_limit_manual == 7) %>% 
...
contour_grid <- plot_grid(contour_fwd_digital, contour_fwd_combo_2day, contour_fwd_combo_7day,
                          contour_bidir_digital, contour_bidir_combo_2day, contour_bidir_combo_7day,
...
path_prefix <- "figures_local/img/si_contour_reff"

# 48% pre-symptomatic
row_height <- 13
save_fig(path_prefix, ".svg", contour_grid, row_height*2, 0.9*3/2, device="svg")
save_fig(path_prefix, ".png", contour_grid, row_height*2, 0.9*3/2, device="png")
