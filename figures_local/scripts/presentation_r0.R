...
# Median scenario
median_path <- "figure_data/si/fig3_baseline_explore_1k_scenario.tsv.gz"
median_data <- suppressMessages(read_tsv(median_path)) %>%
  filter(p_traced_auto == 0)

median_digital_path <- "figure_data/si/fig4_baseline_morer0_1k_scenario.tsv.gz"
...
optim_digital_path <- "figure_data/si/fig4_optimistic_morer0_1k_scenario.tsv.gz"
optim_data_digital <- suppressMessages(read_tsv(optim_digital_path)) %>%
  filter(p_traced_manual == 0 | p_traced_manual == p_traced_auto)

optim_data <- bind_rows(optim_data, optim_data_digital)
...
pessim_digital_path <- "figure_data/si/fig4_pessimistic_morer0_1k_scenario.tsv.gz"
pessim_data_digital <- suppressMessages(read_tsv(pessim_digital_path)) %>%
  filter(p_traced_manual == 0 | p_traced_manual == p_traced_auto)

pessim_data <- bind_rows(pessim_data, pessim_data_digital)
...
prepare_fig3_all <- function(median_data, optim_data, pessim_data,
                            p_traced, trace_neg_sym, contact_limit = NULL, bt_distance = NULL,
                            ttype_exclude = NULL, pt_size = 2,
                            err_width_ctrl = 0.05, hide_legend = TRUE,
  ...
  x_axis_titles <- c(expression(R[0]~"(median scenario)"),
                     expression(R[0]~"(optimistic scenario)"),
                     expression(R[0]~"(pessimistic scenario)"))
  # Make control plots
  ctrl_plots <- lapply(1:3, function(n)
  ...
  # Make R_eff plots
  reff_plots <- lapply(1:3, function(n)
  ...
  # Hide legends
  if (hide_legend){
    ctrl_plots <- lapply(1:length(ctrl_plots), function(n)
      ctrl_plots[[n]] + theme(legend.position = "none"))
    reff_plots <- lapply(1:length(reff_plots), function(n)
      reff_plots[[n]] + theme(legend.position = "none"))
  }
  # Assemble figure
  fig <- plot_grid(ctrl_plots[[1]], reff_plots[[1]],
                   ctrl_plots[[2]], reff_plots[[2]],
                   ctrl_plots[[3]], reff_plots[[3]],
...
fig_7d_090_pre <- prepare_fig3_all(median_data, optim_data, pessim_data, 0.9, TRUE,
                                   7, NULL, y_title_short_reff = FALSE,
                                   ttype_exclude = ttype_exclude)

fig_7d_080_pre <- prepare_fig3_all(median_data, optim_data, pessim_data, 0.8, TRUE,
                                  7, NULL, y_title_short_reff = FALSE,
                                  ttype_exclude = ttype_exclude)

fig_2d_090_pre <- prepare_fig3_all(median_data, optim_data, pessim_data, 0.9, TRUE,
                                  2, NULL, y_title_short_reff = FALSE,
                                  ttype_exclude = ttype_exclude)

fig_2d_080_pre <- prepare_fig3_all(median_data, optim_data, pessim_data, 0.8, TRUE,
                                  2, NULL, y_title_short_reff = FALSE,
                                  ttype_exclude = ttype_exclude)

fig_7d_090_post <- prepare_fig3_all(median_data, optim_data, pessim_data, 0.9, FALSE,
                                   7, NULL, y_title_short_reff = FALSE,
                                   ttype_exclude = ttype_exclude)

fig_7d_080_post <- prepare_fig3_all(median_data, optim_data, pessim_data, 0.8, FALSE,
                                   7, NULL, y_title_short_reff = FALSE,
                                   ttype_exclude = ttype_exclude)

fig_2d_090_post <- prepare_fig3_all(median_data, optim_data, pessim_data, 0.9, FALSE,
                                   2, NULL, y_title_short_reff = FALSE,
                                   ttype_exclude = ttype_exclude)

fig_2d_080_post <- prepare_fig3_all(median_data, optim_data, pessim_data, 0.8, FALSE,
                                   2, NULL, y_title_short_reff = FALSE,
                                   ttype_exclude = ttype_exclude)
...
path_prefix <- "figures/si/all_explore_lowuptake_"
row_height <- 12

save_fig(path_prefix, "7d_090_pre.svg", fig_7d_090_pre, row_height*3*0.85, 2/3 * 0.9)
save_fig(path_prefix, "7d_080_pre.svg", fig_7d_080_pre, row_height*3*0.85, 2/3 * 0.9)
save_fig(path_prefix, "2d_080_pre.svg", fig_2d_080_pre, row_height*3*0.85, 2/3 * 0.9)
save_fig(path_prefix, "2d_090_pre.svg", fig_2d_090_pre, row_height*3*0.85, 2/3 * 0.9)
save_fig(path_prefix, "7d_080_post.svg", fig_7d_080_post, row_height*3*0.85, 2/3 * 0.9)
save_fig(path_prefix, "7d_090_post.svg", fig_7d_090_post, row_height*3*0.85, 2/3 * 0.9)
save_fig(path_prefix, "2d_080_post.svg", fig_2d_080_post, row_height*3*0.85, 2/3 * 0.9)
save_fig(path_prefix, "2d_090_post.svg", fig_2d_090_post, row_height*3*0.85, 2/3 * 0.9)