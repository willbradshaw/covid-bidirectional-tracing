...
data_path <- "figures_local/data/si_ascertainment_baseline_1000_scenario.tsv.gz"
...
# High-uptake
plot_high <- data_processed %>% 
  filter(p_smartphone_overall == 0.8) %>%
  combo_plot()

# Low-uptake
plot_low <- data_processed %>% 
  filter(p_smartphone_overall == 0.53) %>%
  combo_plot()
...
path_prefix <- "figures_local/img/presentation_ascertainment"

plot_scale <- 13
plot_ratio <- 21/plot_scale 

save_fig(path_prefix, "_high.png", plot_high,
         plot_scale, plot_ratio, device="png")
save_fig(path_prefix, "_low.png", plot_low,
         plot_scale, plot_ratio, device="png")
