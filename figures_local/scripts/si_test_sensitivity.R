...
data_path <- "figures_local/data/si_sensitivity_500_scenario.tsv.gz"
...
control_plot_90 <- data_processed %>%
  ...
  scale_x_continuous(name = "Test sensitivity (%)", limits=c(0,1.05),
...
r_eff_plot_90 <- data_processed %>%
  ...
  scale_y_continuous(name = "Average effective reproduction number",
...
plot_scale <- 10
plot_prefix <- "figures_local/img/si_sensitivity"
ggsave(filename=paste0(plot_prefix, ".svg"), plot = out_plot,
       device = "svg", width = plot_scale*3.4,
