...
manual_path <- "figure_data/si/si_manual_wide_500_scenario.tsv.gz"
...
manual_plot_ctrl <- manual_data %>%
  ggplot(aes(x=p_traced_manual, y=p_controlled, 
             ymin = p_controlled_lower, ymax = p_controlled_upper,
             colour = factor(contact_limit_manual),
             linetype = factor(backtrace_distance, levels=c(Inf,0)),
             shape = factor(backtrace_distance, levels=c(Inf, 0)))) +
  ...
  scale_colour_discrete(labels=label_limits_manual, name=NULL) +
...
# Combine together
manual_plot <- plot_grid(manual_plot_ctrl, manual_plot_reff, 
...
path_prefix <- "figures/si/manual_wide"
row_height <- 14
save_fig(path_prefix, ".png", manual_plot, row_height, 0.8*2)
