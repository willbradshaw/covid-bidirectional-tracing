...
data_path <- "figure_data/si/gen_delay_baseline_090_500_scenario.tsv.gz"
...
control_plot_90 <- data_processed %>%
  ...
  facet_grid(backtrace_distance~r0_base,
             labeller=labeller(backtrace_distance=label_backtrace,
                               r0_base = function(x) paste0("R0 = ", x))) +
...
r_eff_plot_90 <- envir_data_processed %>% filter(p_traced_auto != 0.8,
  ...
  scale_x_continuous(name = "% environmental transmission",
...
envir_plot <- plot_grid(control_plot_90, r_eff_plot_90,
...
plot_scale <- 14
plot_prefix <- "figures/si/envir"
ggsave(filename=paste0(plot_prefix, ".svg"), plot = envir_plot,
