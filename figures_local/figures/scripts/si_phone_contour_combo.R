...
data_path <- "figure_data/si/si_phone_heatmap_combo_500_scenario.tsv.gz"
...
heatmap_bi <- data %>% filter(backtrace_distance == Inf) %>%
  ggplot(aes(y=p_data_sharing_auto, x=p_smartphone_overall,
             z=round(p_controlled*100))) +
  #geom_tile() +
  geom_contour_filled(colour="black") +
  geom_text_contour(colour="black", stroke=0.2) +
  # geom_text(aes(y=p_data_sharing_auto, x=p_smartphone_overall, label=round(p_controlled*100)),
  #           colour="black", size=3)+
  scale_x_continuous(name = "% of cases with\nchirping smartphones",
                     breaks = seq(0,1,0.2), labels = function(x) round(x*100)) +
  scale_y_continuous(name = "% of cases sharing data", breaks = seq(0,1,0.2),
                     labels = function(x) round(x*100)) +
  # scale_fill_gradient(low=gradient_low, high="#d95f02", name = "% of outbreaks\ncontrolled",
  #                     labels = function(x) round(x*100), limits=c(0,1)) +
  # scale_fill_manual(values=scales::seq_gradient_pal(low="white", high="#d95f02", 
  #                                                    "Lab")(seq(0,1,length.out=12))) +
  coord_fixed(ratio=1) +
  theme_base + theme(
...
path_prefix <- "figures/si/phone_heatmaps_combo"
row_height <- 14
