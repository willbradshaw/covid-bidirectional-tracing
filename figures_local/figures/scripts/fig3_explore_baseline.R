...
prepare_fig3_baseline <- function(median_data,
                                  p_traced, contact_limit = NULL, bt_distance = NULL,
  ...
  x_axis_titles <- c(expression(R[0]))
  # Make control plots
  ctrl_plots <- lapply(1:1, function(n)
  ...
  # Make R_eff plots
  reff_plots <- lapply(1:1, function(n)
...
fig_7d_080_pre <- data %>% filter(p_traced_manual != 0.9,
                                  contact_limit_manual == 7,
                                  trace_neg_symptomatic == TRUE) %>%
  prepare_fig3_baseline(0.8, 7, NULL, y_title_short_reff = FALSE,
                        ttype_exclude = ttype_exclude)

fig_2d_090_pre <- data %>% filter(p_traced_manual != 0.8,
                                  contact_limit_manual == 2,
                                  trace_neg_symptomatic == TRUE) %>%
  prepare_fig3_baseline(0.9, 2, NULL, y_title_short_reff = FALSE,
                        ttype_exclude = ttype_exclude)

fig_2d_080_pre <- data %>% filter(p_traced_manual != 0.9,
                                  contact_limit_manual == 2,
                                  trace_neg_symptomatic == TRUE) %>%
  prepare_fig3_baseline(0.8, 2, NULL, y_title_short_reff = FALSE,
                        ttype_exclude = ttype_exclude)

fig_7d_090_post <- data %>% filter(p_traced_manual != 0.8,
                                  contact_limit_manual == 7,
                                  trace_neg_symptomatic == FALSE) %>%
  prepare_fig3_baseline(0.9, 7, NULL, y_title_short_reff = FALSE,
                        ttype_exclude = ttype_exclude)

fig_7d_080_post <- data %>% filter(p_traced_manual != 0.9,
                                   contact_limit_manual == 7,
                                   trace_neg_symptomatic == FALSE) %>%
  prepare_fig3_baseline(0.8, 7, NULL, y_title_short_reff = FALSE,
                        ttype_exclude = ttype_exclude)

fig_2d_090_post <- data %>% filter(p_traced_manual != 0.8,
                                   contact_limit_manual == 2,
                                   trace_neg_symptomatic == FALSE) %>%
  prepare_fig3_baseline(0.9, 2, NULL, y_title_short_reff = FALSE,
                        ttype_exclude = ttype_exclude)

fig_2d_080_post <- data %>% filter(p_traced_manual != 0.9,
                                   contact_limit_manual == 2,
                                   trace_neg_symptomatic == FALSE) %>%
  prepare_fig3_baseline(0.8, 2, NULL, y_title_short_reff = FALSE,
                        ttype_exclude = ttype_exclude)
...
path_prefix <- "figures/si/baseline_explore_"
row_height <- 14

save_fig(path_prefix, "7d_080_pre.png", fig_7d_080_pre, row_height*1.1, 2 * 0.8)
save_fig(path_prefix, "2d_080_pre.png", fig_2d_080_pre, row_height*1.1, 2 * 0.8)
save_fig(path_prefix, "2d_090_pre.png", fig_2d_090_pre, row_height*1.1, 2 * 0.8)
save_fig(path_prefix, "7d_080_post.png", fig_7d_080_post, row_height*1.1, 2 * 0.8)
save_fig(path_prefix, "7d_090_post.png", fig_7d_090_post, row_height*1.1, 2 * 0.8)
save_fig(path_prefix, "2d_080_post.png", fig_2d_080_post, row_height*1.1, 2 * 0.8)
save_fig(path_prefix, "2d_090_post.png", fig_2d_090_post, row_height*1.1, 2 * 0.8)